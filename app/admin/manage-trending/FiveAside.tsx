...
export default function FiveAside({ trending }: FiveAsideProps) {
  ...
  const topTrendingMarkets: MarketTrending = upcomingTrending.reduce(
                ...
                <AccordionItem value={key}>
                  <AccordionTrigger>
                    <div className="flex justify-center items-center space-x-2">
                      <span>{`${key}`}</span>
                      <span className="flex items-center justify-center text-smallest text-center px-[7px] bg-primary rounded-full">
                        {value.length}
                      </span>
                    </div>
                  </AccordionTrigger>
